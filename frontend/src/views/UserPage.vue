...
onMounted(async () => {
  ...
  const res = await fetch(`http://localhost:3000/api/users/${props.id}`, { headers })
  user.value = await res.json()

  // initialize form
  Object.assign(form, {
    pattern_id: user.value.pattern_id || '',
    ticker: user.value.ticker || '',
    timeframe: user.value.timeframe || '',
    direction: user.value.direction || 'long',
    leverage: user.value.leverage || 1,
    entry_plan: user.value.entry_plan || 0,
    stop_loss: user.value.stop_loss || 0,
    take_profit: user.value.take_profit || 0,
    signals: user.value.signals || [],
    executions: user.value.executions || [],
...
async function submitForm() {
  ...
  try {
    ...
    message.value = "Error saving user: " + err.message
  } finally {
    saving.value = false
  }
}
...
<div class="max-w-6xl mx-auto p-6 space-y-8">
  ...
  <div class="flex items-center justify-between mb-6">
    <h1 class="text-3xl md:text-4xl font-extrabold gradient-text">
      {{ user.ticker?.toUpperCase() }} — {{ user.pattern_name }}
    </h1>
    <button @click="showEdit = !showEdit"
      ...
      {{ showEdit ? 'Close Editor' : 'Edit User' }}
    </button>
  </div>
